...
fn train(cfg: Cfg) {
    ...
    for step in 0..cfg.steps {
        ...
        let f_t: Tensor<B, 1> = net.forward(r_t.clone()).squeeze::<1>(1);
        let f_p: Tensor<B, 1> = net.forward(r_p).squeeze::<1>(1);
        let f_m: Tensor<B, 1> = net.forward(r_m).squeeze::<1>(1);
        let fp_tensor = (f_p.clone() - f_m.clone()) / (2.0 * h);
        let fpp_tensor = (f_p - f_t.clone() * 2.0 + f_m) / (h * h);

        // ---- exotic-mass integrand: (v²/12) (f')² r² · 4π ----
        let r1: Tensor<B, 1> = r_t.clone().squeeze::<1>(1);
        ...
        // ---- pin f(R) = 0.5 ----
        let f_at_r = net.forward(r_pin.clone()).squeeze::<1>(1);
